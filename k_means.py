...
def calculate_centers(
    ...
    for i in range(num_centers):
        ...
        new_size = np.sum(match)
        if new_size != 0:
            new_centers[i, :] = np.sum(data[match], axis=0) / new_size
...
def lloyd_algorithm(
    ...
    """
    norm_dif = np.inf
    new_centers = data[:num_centers, :]
    errors = []

    while norm_dif > epsilon:
        old_centers = new_centers.copy()
        cluster = cluster_data(data, new_centers)
        new_centers = calculate_centers(data, cluster, num_centers)
        norm_dif = np.linalg.norm(new_centers - old_centers)
        error = calculate_error(data, new_centers)
        errors.append(error)

    return new_centers, errors
