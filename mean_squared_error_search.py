    ...
    with torch.no_grad():
        for (x, y) in dataloader:
            ...
            for j in range(len(class_predict)):
                ...
                if torch.argmax(y,dim=1)[j] == class_predict[j]:
                    correct_count += 1
...
def mse_parameter_search(
    ...
    model_network_1 = nn.Sequential(LinearLayer(2, 2))
    model_network_2 = nn.Sequential(LinearLayer(2, 2), SigmoidLayer(), LinearLayer(2, 2))
    model_network_3 = nn.Sequential(LinearLayer(2, 2), ReLULayer(), LinearLayer(2, 2))
    model_network_4 = nn.Sequential(LinearLayer(2, 2), SigmoidLayer(), LinearLayer(2, 2), ReLULayer(),
                                    LinearLayer(2, 2))
    model_network_5 = nn.Sequential(LinearLayer(2, 2), ReLULayer(), LinearLayer(2, 2), SigmoidLayer(),
                                    LinearLayer(2, 2))
...
def main():
    ...
    for nn_name, data in mse_configs.items():
        score = accuracy_score(data['model'], dataloader_test)
        print(f'{nn_name} accuracy score:', score)
        plot_model_guesses(dataloader_test, data['model'], title=f'MSE Search {nn_name}')
